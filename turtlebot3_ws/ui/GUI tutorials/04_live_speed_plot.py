#!/usr/bin/env python3
"""
04 — Live speed plot (|linear x|) + ODOM visualization (PyQt5)

Adds a pyqtgraph chart (last ~60s) to the Step-03 app.
- Tele-op (buttons + WASD/Arrows) publishes /cmd_vel at 50 Hz
- Subscribes /odom → pose + velocity, draws rectangle robot + path
- Canvas reused from Step-01 with Step-03 additions
- Safe shutdown on close

Run:
  source /opt/ros/humble/setup.bash
  /usr/bin/python3 04_live_speed_plot.py
"""
...
class Canvas(QWidget):
    def __init__(self):
        ...
        self._state_provider = None  # fn() -> (x,y,yaw,path)
    ...
    def mouseMoveEvent(self, e):
        if self._dragging:
            d=e.pos()-self._last_mouse; self.offset_x+=d.x(); self.offset_y+=d.y(); self._last_mouse=e.pos(); self.update()
    ...
    def paintEvent(self,_):
        ...
        if self._state_provider:
            ...
            self._draw_path(p, path); self._draw_robot_rect(p, x, y, yaw)
    ...
    def _draw_path(self,p,path):
        ...
        for px,py in path:
            ...
            last=pt
    def _draw_robot_rect(self,p,x,y,yaw,L=0.18,W=0.14):
...
class OdomTeleopNode(Node):
    def __init__(self):
        ...
        self.path=[]; self._last=None; self._min_spacing=0.03; self._max=3000
    ...
    def _odom_cb(self,msg:Odometry):
        ...
        if self._last is None or (self.x-self._last[0])**2+(self.y-self._last[1])**2>=self._min_spacing**2:
...
class SpeedPlot(QWidget):
    """Live plot of |v_lin| over last ~60s (optional if pyqtgraph missing)."""
    def __init__(self, node: OdomTeleopNode):
        super().__init__()
        self.n=node; self.ts=deque(); self.vs=deque(); self.window_s=60.0
        L=QVBoxLayout(self)
        if HAVE_PG:
            ...
            L.addWidget(QLabel("pyqtgraph not installed (sudo apt install python3-pyqtgraph).")); self.plot=None
...
class Main(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("04 — Live speed plot + ODOM viz")
        self.resize(1250, 780)

        # ROS
        rclpy.init(); self.node=OdomTeleopNode()
        ...
        hud=QGroupBox("Robot State (/odom)"); hL=QVBoxLayout(hud)
        ...
        self.speed_plot=SpeedPlot(self.node); pL.addWidget(self.speed_plot); L.addWidget(plot_box,1)
        L.addStretch(0)
    ...
    def _tick(self):
        ...
        self.speed_plot.step()

    def closeEvent(self,event): self._clean_shutdown(); event.accept()
    def _clean_shutdown(self):
        ...
        try:
            ...
            if self.ros_thread.is_alive(): self.ros_thread.join(timeout=2.0)
